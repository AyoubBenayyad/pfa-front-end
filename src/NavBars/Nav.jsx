...
const connection = useConnection();
  ...
  async function fetchUsers(v1,v2) {
    try {
        ...
        const response = await fetch("http://localhost:8080/api/v1/search", requestOptions);
  ...
  return (
    ...
    <div className="container flex justify-between h-6 mx-auto">
      ...
      <div class="relative">
        ...
        <input type="search" onChange={SearchChange}  className="block w-96 h-8  ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search profils"  />
            ...
            {isSearching && (
              <>
               <div
            className="absolute right-0 z-20 w-96 max-h-52 py-2 mt-2 overflow-y-auto origin-top-right bg-white rounded-md shadow-xl dark:bg-gray-800"
          >
            {!isloading  && searchedUsers.length>0 && searchedUsers.map((users,index)=>(
              <React.Fragment key={index}>

                        <div
                        key={users.id}
                      className="flex items-center px-3 -mt-2  text-sm text-stone-950 transition-colors duration-300 transform dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                    >
                      <button
